			...
			Fields::Named(FieldsNamed {named, ..}) => {
				let expr = named.iter().map(|f| {
					...
					match ty.as_str() {
						...
						_ => quote!(writer.write(&self.#f_name.to_be_bytes())?;) // we hope that only numerical types will get here (<-- trash)
					}
				});
				...
				let doc_id = format!("Returned [`crate::protocol::PacketID::{}`] as `u8`", ident.to_string());
				...
				quote! {
					...
					impl crate::protocol::ServerPacket for #ident {
	                    ...
	                    fn compose(&self) -> Result<byte_order::NumberReader<std::io::Cursor<Vec<u8>>>, Box<dyn std::error::Error>> {
			...
			Fields::Named(FieldsNamed {named, ..}) => {
					...
					match ty {
						...
						"async_std :: net :: SocketAddr" => quote! {
	                        #f_name: {
								...
								format!("{}.{}.{}.{}:{}", ip[0], ip[1], ip[2], ip[3], port).parse()?
	                        },
	                    },
						"[u8 ; 3]" => todo!(), // uint24_le
						"[u8 ; 16]" => quote! {
				...
				quote! {
	                impl crate::protocol::ClientPacket for #ident {
	                    ...
	                    fn parse(mut reader: byte_order::NumberReader<std::io::Cursor<Vec<u8>>>) -> Result<Self, Box<dyn std::error::Error>> {
							use std::io::Cursor;
