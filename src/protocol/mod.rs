...
pub trait ClientPacket: Sized {
	fn id() -> u8;
	fn parse(reader: NumberReader<Cursor<Vec<u8>>>) -> Result<Self, Box<dyn Error>>;
}

pub trait ServerPacket: Sized {
	fn id() -> u8;
	fn compose(&self) -> Result<NumberReader<Cursor<Vec<u8>>>, Box<dyn Error>>;
}
...
mod tests {
	...
	fn server_packet_new() {
		let ping = ConnectedPing::new(true,
		                   1u8,
		                   2i8,
		                   3u16,
		                   4i16,
	...
	fn server_packet_compose() {
		...
		let eq_packet = Vec::<u8>::from([
			...
			0x00, 0x0b /* len */, 0x54, 0x65, 0x73, 0x74, 0x20, 0x73, 0x74, 0x72, 0x69, 0x6e, 0x67, // string
	...
	fn client_packet_parse() {
		let raw_packet = Vec::<u8>::from([ // The packet from the client is stripped of the ID field after processing
			0x01, // bool
			0x01, // ubyte
			0x02, // byte
			0x00, 0x03, // ushort
